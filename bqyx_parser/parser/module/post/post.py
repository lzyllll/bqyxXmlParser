...
def run(xml_dir: Path | None = None, out_put_dir: Path | None = None) -> None:
    ...
    output_path = out_put_dir / "postData.json"

    logger.info("解析 %s", xml_path)
    result = parse_element_by_factory(load_xml(xml_path), create_post_factory())
    result = result.get('father').get('body')
    for post_data in result:
        post_data['lv'] = post_data['postExpMul']
