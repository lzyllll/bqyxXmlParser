...
class FatherParser(ElementParser):
    ...
    def parse(self, element: Element) -> list[dict[str, Any]]:
        ...
        for child in element:
            # 过滤非单位：必须包含 hitRect（或通过 fixed 继承基础单位）
            if child.tag == "body" and (child.find("hitRect") is not None or "fixed" in child.attrib):
                parsed = self.parser_element(child)
                # 过滤掉没有 name 的body，为非单位
                if parsed.get("name") is None:
                    continue
                parsed['father'] = get_father_name(element)
                parsed['fatherCnName'] = element.attrib.get('cnName')
                if parsed.get('father') == 'pet' and not parsed.get('cnName', '').startswith('尸宠'):
                    parsed['cnName'] = '尸宠' + parsed['cnName']


                merged.append(parsed)

        return merged
def create_normal_factory():
...
def run(xml_dir: Path | None = None, out_put_dir: Path | None = None) -> None:
    ...
    bodys_paths = [
        file for file in xml_dir.iterdir() 
        if file.is_file() and file.suffix == ".xml" and file.name != "hero.xml"
    ]
    
    output_path = out_put_dir / "normal.json"
    result = []

    logger.info("解析 body (normal) 列表...")
    for body_path in bodys_paths:
        res = parse_element_by_factory(load_xml(body_path), create_normal_factory())
        result.extend(res)
