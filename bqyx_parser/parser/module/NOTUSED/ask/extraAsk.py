...
def create_extraAsk_factory():
    factory = create_factory()
    return factory
if __name__ == "__main__":
    xml_dir = Path(r"compiled\v3671\xml")
    out_put_dir = Path(r"output\v3671\resource\ask")
    out_put_dir.mkdir(parents=True, exist_ok=True)

    xml_path = xml_dir / "extraAsk.xml"
    output_path = out_put_dir / "extraAskClass.json"
    resource_path = Path(r"D:\bqyx\rs\resource\ask\extraAskClass.json")

    logger.info("解析 %s", xml_path)
    result = parse_element_by_factory(load_xml(xml_path), create_extraAsk_factory())
