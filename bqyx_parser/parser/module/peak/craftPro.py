...
def run(xml_dir: Path | None = None, out_put_dir: Path | None = None) -> None:
    ...
    if out_put_dir is None:
        out_put_dir = Path(r"output\v3671\resource\peak")
    out_put_dir.mkdir(parents=True, exist_ok=True)

    xml_path = xml_dir / "_00_SilverShip.xml"
    output_path = out_put_dir / "craftPro.json"

    logger.info("解析 %s", xml_path)
    result = parse_element_by_factory(load_xml(xml_path), create_craftPro_factory())
    result = result.get('father')[0].get('pro')
