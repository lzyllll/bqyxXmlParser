...
def create_petStrengthen_factory():
...
def run(xml_dir: Path | None = None, out_put_dir: Path | None = None) -> None:
    ...
    if out_put_dir is None:
        out_put_dir = Path(r"output\v3671\resource\pet")
    out_put_dir.mkdir(parents=True, exist_ok=True)

    xml_path = xml_dir / "petStrengthen.xml"
    output_path = out_put_dir / "petStrengthen.json"

    logger.info("解析 %s", xml_path)
    result = parse_property(xml_path)
