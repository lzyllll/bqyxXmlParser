...
def create_partsProperty_factory():
...
def run(xml_dir: Path | None = None, out_put_dir: Path | None = None) -> None:
    ...
    if out_put_dir is None:
        out_put_dir = Path(r"output\v3671\resource\parts")
    out_put_dir.mkdir(parents=True, exist_ok=True)

    xml_path = xml_dir / "partsProperty.xml"
    output_path = out_put_dir / "partsProperty.json"

    logger.info("解析 %s", xml_path)
    result = parse_element_by_factory(load_xml(xml_path), create_partsProperty_factory())
    result = result.get("father").get("pro")
