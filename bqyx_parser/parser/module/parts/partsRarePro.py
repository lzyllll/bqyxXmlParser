...
def create_partsRarePro_factory():
...
def run(xml_dir: Path | None = None, out_put_dir: Path | None = None) -> None:
    ...
    xml_path = xml_dir / "partsRarePro.xml"
    output_path = out_put_dir / "partsRarePro.json"

    logger.info("解析 %s", xml_path)
    result = parse_element_by_factory(load_xml(xml_path), create_partsRarePro_factory())
    result = result.get('pro')
