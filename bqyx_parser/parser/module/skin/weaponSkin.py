...
def parse_weapon_skin(xml_path: str | Path) -> list[dict[str, Any]]:
    """使用统一安全加载函数 load_xml 解析 weaponSkin.xml，返回副手武器皮肤与形态列表。"""
    root = load_xml(xml_path)
    result: list[dict[str, Any]] = []

    for gather in root.findall(".//gather"):
        gather_name = gather.attrib.get("name", "weapon")
...
def run(xml_dir: Path | None = None, out_put_dir: Path | None = None) -> None:
    ...
    if out_put_dir is None:
        out_put_dir = Path(r"output\v3671\json\skin")
    out_put_dir.mkdir(parents=True, exist_ok=True)

    xml_path = xml_dir / "weaponSkin.xml"
    output_path = out_put_dir / "weaponSkin.json"

    logger.info("解析 %s", xml_path)
    skins = parse_weapon_skin(xml_path)
    with open(output_path, "w", encoding="utf-8") as f:
        json.dump(skins, f, ensure_ascii=False, indent=2)
    logger.info("已保存 %s (共 %d 条副手皮肤定义)", output_path, len(skins))
