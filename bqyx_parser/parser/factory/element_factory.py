...
from bqyx_parser.parser.element.default_parser import EmptyElementParser
#只是用于类型检查，方便提示
if TYPE_CHECKING:
...
class ElementParserFactory:
    ...
    def parse_element(self, child: etree._Element) -> Any:
        """解析元素（公开的解析方法）"""
        parser = self.get_parser(child)
        return parser.parse(child)
    
    def get_parser(self, child: etree._Element) -> 'ElementParser':
        """获取适合的解析器（按优先级顺序检查）"""
