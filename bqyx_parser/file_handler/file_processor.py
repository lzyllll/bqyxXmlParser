...
class FileProcessor:
    ...
    def _convert_filename(self, file_path: Path) -> str:
        ...
        if match:
            ...
            # 获取不带数字前缀的部分
            clean_name = remaining.replace('.bin', '.xml')
        else:
            # 直接替换后缀
...
=================================
'''
def copy_dir(dir:Path,output_dir:Path,icon_dict:dict):
    for file in dir.iterdir():
        if file.stem in icon_dict:
            #根据映射重命名 比如 coat_icon => aintSuit_belt
            new_path = file.with_stem(icon_dict.get(file.stem))
            #复制
            with open(file, 'rb') as source_file:
                with open(output_dir / new_path.name, 'wb') as dest_file:
                    dest_file.write(source_file.read())

def copy_equip_svg(target:Path,output_dir:Path):
    icons_dict = dict(
        zip(
    ...
    svg_dir = target / 'shapes'
    copy_dir(svg_dir,output_dir,icons_dict)

def copy_equip_image(target:Path,output_dir:Path):
    '''
    根据映射， 复制
    '''
    icons_dict = dict(
    ...
    svg_dir = target / 'images'
    copy_dir(svg_dir,output_dir,icons_dict)       
def copy_fashion_svg(target:Path,output_dir:Path):
    """根据映射， 复制"""
    icons_dict = dict(
        zip(
            ["fashion_icon"],
...
def copy_fashion_image(target:Path,output_dir:Path):
    """根据映射， 复制"""
    icons_dict = dict(
        zip(
            ["fashion_icon"],
    ...
    copy_dir(image_dir,output_dir,icons_dict)

if __name__ == "__main__":
    file_processor = FileProcessor()
    file_processor.rename_images_and_svgs(Path(r'C:\Users\lzy\Desktop\dev-bqyx\python\request_image\compiled\equip\data\armyCommander'))